...
if __name__ == '__main__':
	...
	sentences = map(lambda x: x['metadata']['metadata'].get('description', ''), items)
	print("Done one pass of mapping sentences")
	sentences_ = map(lambda x: doc2vec.TaggedDocument(words=x[1].split(), tags=[x[0]]), enumerate(sentences))
	print("Done another pass of mapping sentences")
	model = doc2vec.Doc2Vec(alpha=0.1, min_alpha=0.01)
	model.build_vocab(documents=sentences_)
	model.train(sentences_, total_examples=model.corpus_count, epochs=100)
	model.save('doc2vec_100.model')
	print('Model Saved')
