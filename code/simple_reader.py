...
def title_generator(collection, batch_size):
    i = 0
    titles = []
    ...
    try:
        ...
        max_len = max(map(lambda x: len(x.split(' ')), titles))
        np.save('max_seq_length.npy', max_len)
        with open('tokenizer.pickle', 'wb') as handle:
            pickle.dump(t, handle, protocol=pickle.HIGHEST_PROTOCOL)
    print batch_size
    while (True):
        i %= len(titles) // batch_size
        print len(titles[i * batch_size: min(len(titles), (i + 1) * batch_size)])
        encoded_titles = t.texts_to_matrix(titles[i * batch_size: min(len(titles), (i + 1) * batch_size)])
        i += 1
        yield encoded_titles
...
def mongoDBgenerator(collection, d2vmodel, numComments, which, batch_size, validation_ratio, use_audio, use_video
        , use_desc, use_metadata, use_comments, use_channel, use_titles=False):
    print("Found", collection.find({"which":which}).count(), "records")
    print batch_size
    if (use_titles):
        title_gen = title_generator(collection, batch_size)
    while True:
        i = 0
        X_metadata = []
        X_audio = []
